...
for iC = 1:size(S,1) %length(S)
    for iM = 1:size(S,2)
        ...
        if ~isempty(S(iC, iM).rgrsMION_resample)
            tic
            [Rvals, Pvals] = corr(reshape(dataBOLD.catmvoltc{iM}, nVox, nt)', S(iC, iM).rgrsMION_resample','rows','complete');
            
            S(iC, iM).mapR = reshape(Rvals, [nx, ny, nz]).*(-1); % because of MION
            S(iC, iM).mapP = reshape(Pvals, [nx, ny, nz]);
