function [] = doClusteringCorrMap_multipleSubj_corrcrit(flagParallel, flagSave)
% 2017/01/25 SHP
...
% Set directories 
setNameSubjNeural = {'Dav', 'Tor', 'Rho', 'Sig', 'Spi', 'Mat', 'Dan'}; %{'Tor', 'Rho', 'Sig', 'Spi'};
nameSubjBOLD ='Art'; % 'Ava'; %'Art'; % 'Ava'; %'Art'; %'Ava'; %'Art';
dirDataHome = fullfile(dirProcdata, 'parksh'); %'/data/parks20/procdata/NeuroMRI/'; %fullfile(dirProcdata, 'parksh');
...
%     end
    ...
    if flagSave
        save(fullfile(dirDataBOLD, sprintf('Clustering_%s%sMovie123_new_masked_critCorr1.mat', cell2mat(setNameSubjNeural), nameSubjBOLD)), ... %nameSubjNeural, nameSubjBOLD)),...
...
save(fullfile(dirDataBOLD, sprintf('Clustering_%s%sMovie123_new_masked_critCorr1_eval.mat', cell2mat(setNameSubjNeural), nameSubjBOLD)), ... %nameSubjNeural, nameSubjBOLD)),...
        'eva_clusteringSU');
