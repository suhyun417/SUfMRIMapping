function [] = doClusteringCorrMap_pcares_multipleSubj_multiPatches(flagParallel, flagSave)
% 2017/01/22 SHP
...
for iK = 1:length(setK)
    ...
    [IDX_Vox, C_Vox, SUMD_Vox] = kmeans(matR_SU_all_moviemask, K, 'Replicates', 5, 'Options', opts); % numReplicates, 'Options', opts);
...
for iK=1:length(setK)
    plot(iK+1, Clustering_moviemask.resultKMeans(iK).SU_sumD, 'ko', 'MarkerSize', 10, 'LineWidth', 2)
    hold on
end
xlim([1 16])
...
eva_clusteringSU = evalclusters(matR_SU_all_moviemask', 'kmeans', 'gap', 'KList', paramClustering_global.setK, 'Distance', 'sqEuclidean');
save(fullfile(dirDataBOLD, sprintf('Clustering_%s%sMovie123_pcares_masked_eval.mat', cell2mat(setNameSubjNeural), nameSubjBOLD)), ... %nameSubjNeural, nameSubjBOLD)),...
        'eva_clusteringSU');
fprintf(1, '...done! Evaluation results saved. \n')

fprintf(1, 'Performing gap statistics on Voxel clustering... \n');
eva_clusteringVox = evalclusters(matR_SU_all_moviemask, 'kmeans', 'gap', 'KList', paramClustering_global.setK, 'Distance', 'sqEuclidean'); 
fprintf(1, '...done! Evaluation results saved. \n')
save(fullfile(dirDataBOLD, sprintf('Clustering_%s%sMovie123_pcares_masked_eval.mat', cell2mat(setNameSubjNeural), nameSubjBOLD)), ... %nameSubjNeural, nameSubjBOLD)),...
