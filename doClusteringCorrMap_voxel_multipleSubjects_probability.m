function [] = doClusteringCorrMap_voxel_multipleSubjects_probability(flagParallel, flagSave)
% 2017/01/25 SHP
...
%     diary off 
    % Cluster voxels based on 50 singel unit correlation
    [IDX_Vox, C_Vox, SUMD_Vox] = kmeans(matR_SU_all_moviemask, K,...
...
%     Clustering_brainmask.resultKMeans(iK).Vox_sumD = SUMD_Vox;
    ...
    if flagSave
        save(fullfile(dirDataBOLD, sprintf('Clustering_%s%sMovie123_new_masked_voxel_probability.mat', cell2mat(setNameSubjNeural), nameSubjBOLD)), ... %nameSubjNeural, nameSubjBOLD)),...
