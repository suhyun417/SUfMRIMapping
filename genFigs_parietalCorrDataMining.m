...
%% Above all saved in a temporary mat file, so load it to retrieve data
load('/nifvault/procdata/parksh/_macaque/tempParietalCorr.mat')

%% Maybe first look at the PCA?
catCorr = cat(1, catCorrArea);


[coeff, score, latent, tsquared, explained] = pca(catCorr');
size(coeff)
size(score)
explained(1:10)
cumsum(explained(1:10))
plot(score(:,1), score(:,2))

% catCorr{iArea} = matCorr;
% catCorr_mean{iArea} = mean(matCorr);

%% Copy and paste from old code looking at subcortical regions..
