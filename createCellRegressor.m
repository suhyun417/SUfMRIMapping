function S = createCellRegressor(dirDataNeural, setCellIDs, setMovIDs, flagConcat)
...
%       count = count+1;
      ...
      S(iChan).movID(iMov) = cur_movID;
      [S(iChan).matsdf{iMov}, S(iChan).mnsdf(:,iMov)] = computeMeanSDF(dirDataNeural, filename); % averaged SDF across trials
      fprintf(1, 'cell: %s (%d/%d), movie: %d \n', S(iChan).cellID, iChan, length(setCellIDs), setMovIDs(iMov))
      
%       if ~flagConcat
%           [S(iChan).rgrsMION, S(iChan, iMov).rgrsMION_resample] = computeMIONrgrs(S(iChan, iMov).mnsdf, timeResNeural_sec, fMRI_TR_sec);
%       end
  ...
  fprintf(1, 'cell: %s (%d/%d), Concatenated across %d movies \n', S(iChan).cellID, iChan, length(setCellIDs), length(setMovIDs))
