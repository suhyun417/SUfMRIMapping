function [] = genSwarm_computeCorrcoeffCI(nameSubjNeural, nameSubjBOLD)

% computeCorrcoeffCI(nameSubjNeural, nameSubjBOLD, iChan, idVox_start)

flagRecompile = 1;

% Load the data
dirDataHome ='/data/parks20/procdata/NeuroMRI'; % Biowulf 
dirDataNeural = fullfile(dirDataHome, nameSubjNeural);
dirDataBOLD = fullfile(dirDataHome, nameSubjBOLD);
filenameNeural =sprintf('neuraltc4computeCorrcoeffCI_%s_10hz.mat', nameSubjNeural); % sprintf('neuraltc4computeCorrcoeffCI_%s.mat', nameSubjNeural);
...
%     delete(fullfile(script_path, '*'))
    ...
    mcc2('-m', '-R', '-nodisplay', '-R', '-singleCompThread', '-d', script_path, 'computeCorrcoeffCI.m')
end
    ...
    for ii = 1:size(setChan,2)
        ...
        for jj = 1:length(setStartVox)
            ...
            command_list = [command_list ...
                ...
                matlab_path,...
                nameSubjNeural, nameSubjBOLD, curChan, curStartVox, flagRawSDFShuffle)]; % this command will ask for a different save directory for every channel
        end
    end
    % end
    
    % write the commands into a swarm file
    swarmFileName = sprintf('computeCorrcoeffCI_%s%s_cell%d_%d.swarm', nameSubjNeural, nameSubjBOLD,...
