...
allFaceROIsInd = matAllFaceROIs*[1:size(faceROIs, 2)]'; %matAllFaceROIs*[1:8]'; %sum(matAllFaceROIs,2); %matAllFaceROIs*[1:8]';
...
targetMapVol.vol = allFaceROIsInd_fun;
targetMapVol.fname = sprintf('%s_ROI_%s+orig.BRIK', nameSubjBOLD, 'AllFaceROI_bothH');%
...
%     curFaceROI=decimate3D(faceROIs(iFR).vol3D, resizeFactor, .25); % turn anat_res ROIs into func_res ROIs
    ...
    fname = targetMapVol.fname;  %  fname = sprintf('%s_ROI_%s+orig.BRIK', nameSubjBOLD, faceROIs(iFR).name(3:end));%
%     curFaceROI=decimate3D(faceROIs(iFR).vol3D, resizeFactor, .25);
    DSP.proc.fncvol_3d = targetMapVol.vol;
