...
set(fig_matTS_sites, 'Color', 'w', 'PaperPositionMode', 'auto', 'Position', [500 30 90 1030]);
imagesc(matTS_FP.catAreaID(indSortChan))
colormap(cMap_Area)
...
pairArea = matTS_FP.catAreaID([iset jset]);
...
P = plot(2.4:2.4:900, matTS_norm(:, setPairHighRDiffArea_indChan(iPP,:)), 'LineWidth', 1.5);
P(1).Color = cMap_Area(matTS_FP.catAreaID(setPairHighRDiffArea_indChan(iPP,1)), :);
P(2).Color = cMap_Area(matTS_FP.catAreaID(setPairHighRDiffArea_indChan(iPP,2)), :);
set(gca, 'TickDir', 'out', 'box', 'off')
legend(matTS_FP.catChanID(setPairHighRDiffArea_indChan(iPP,:)))
print(gcf, fullfile(dirFig, sprintf('matTS_FR_TR_exampleTSPair_%d_indChan%d%d', ...
    iPP, setPairHighRDiffArea_indChan(iPP,1), setPairHighRDiffArea_indChan(iPP,2))), '-depsc')
end
