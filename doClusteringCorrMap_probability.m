function [] = doClusteringCorrMap_probability()
...
% Set directories 
nameSubjNeural = 'Spi'; %'Tor'; %'Spi'; %'Tor';
...
for iK = 1:length(setK)
    ...
    SU_indCluster_moviemask = NaN(length(validChanIndex_clustering), numRepeat);
    SU_sumD_moviemask = NaN(K, numRepeat);
    
    SU_indCluster_brainmask = NaN(length(validChanIndex_clustering), numRepeat);
    ...
    Clustering_moviemask.resultKMeans(iK).SU_indCluster = SU_indCluster_moviemask;
    Clustering_moviemask.resultKMeans(iK).SU_sumD = SU_sumD_moviemask;
    
    Clustering_brainmask.resultKMeans(iK).SU_indCluster = SU_indCluster_brainmask;
    Clustering_brainmask.resultKMeans(iK).SU_sumD = SU_sumD_brainmask;
    
end
save(fullfile(dirDataNeural, sprintf('Clustering_%s%sMovie123_new_masked_probability.mat', nameSubjNeural, nameSubjBOLD)),...
