% saveClusterMaps_Surface_voxel_VoxTS.m
%
...
load(fullfile(dirDataBOLD, sprintf('ClusteringVoxTS_%sMovie123.mat', nameSubjBOLD)))
...
for iK = 1:length(paramClustering.setK)
    ...
    % 1. brain mask
    voxelClusterMap_brainmask = zeros(size(brainmask_vec));
    voxelClusterMap_brainmask(brainmask_vec) = Clustering_brainmask.resultKMeans(sortTargetK-1).Vox_indCluster;
    voxelClusterMap_brainmask_vol = reshape(voxelClusterMap_brainmask, [nx, ny, nz]); % each voxel now has cluster id
    
    % 2. movie mask
    voxelClusterMap_moviemask = zeros(size(moviemask_vec));
    voxelClusterMap_moviemask(moviemask_vec) = Clustering_moviemask.resultKMeans(sortTargetK-1).Vox_indCluster;
    ...
    voxelClusterMap_moviemask_vol = reshape(voxelClusterMap_moviemask, [nx, ny, nz]); % each voxel now has cluster identity (integers)
    
    
    % Save it into BRIK/HEAD format to plot it onto surface
    cd /projects/parksh/NeuralBOLD/analysis/BlockAna/
    blockana;
    S_neuralRegressor('Tor', nameSubjBOLD); % just to get the global structure
    ...
    for iMask = 1:2
        ...
        switch iMask
            case 1 % unmasked (everything within brain)
                fileHead = sprintf('%s_brainmask', nameSubjBOLD);
                DSP.proc.fncvol_3d = voxelClusterMap_brainmask_vol; %mapR_Cluster(:,:,:,iK).*brainMask_BlockAna3D;
            case 2 % masked
                fileHead = sprintf('%s_moviemask', nameSubjBOLD); %sprintf('new_maskedSignificant_%s', nameSubjNeural);% sprintf('new_masked_%s', nameSubjNeural);
                DSP.proc.fncvol_3d = voxelClusterMap_moviemask_vol; %mapR_Cluster(:,:,:,iK).*movieDrivenAmp.mask_amp1; %reshape(mapR, [nx, ny, nz]).*movieDrivenAmp.mask_amp1;
        end
        
        %         fname = sprintf('%s_Cluster%d_%dMeans_Art_AvgCorrMapMovie123_noFiltering+orig.BRIK', fileHead, iK, sortTargetK);%
        fname = sprintf('%s_VoxTSClusters_%d_Movie123_noFiltering+orig.BRIK', fileHead,sortTargetK);%
