...
ylabel('Cells')
% 
print(gcf, fullfile(dirFig, sprintf('kmeans_clusteredSU_sortedK%d_comparison_new', sortTargetK)), '-r150', '-dtiff');
...
ylabel('Cells')
print(gcf, fullfile(dirFig, sprintf('kmeans_clusteredSU_sortedK%d_newCorrMaps', sortTargetK)), '-r150', '-dtiff');
...
dirMovie = '/procdata/parksh/Stimulus/Movies/Rhesus'; %'/procdata/parksh/Movies';
...
%     locHigh = find(meanFRCluster_zscore(:,iClust)<critHigh);
    ...
    reverseCorrMov(1:length(locHigh)) = struct('cdata',zeros(videoObj1.Height, videoObj1.Width, 3,'uint8'), 'colormap',[]);
    ...
    % Write a movie
    fileName = sprintf('scenesMov123_SummedResp_DavSpiMatDan_zCrit%d.avi', critHigh);
