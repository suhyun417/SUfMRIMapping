...
numRepeat = 1000; % 100; % number of repetition for entire clustering
...
%    Clustering_moviemask_valid.resultKMeans(iK).Vox_sumD = Vox_sumD_moviemask;
    ...
    if flagSave
        save(fullfile(dirDataBOLD, sprintf('Clustering_%s%sMovie123_new_masked_probability_1000_critCorr1.mat', cell2mat(setNameSubjNeural), nameSubjBOLD)), ... %nameSubjNeural, nameSubjBOLD)),...
...
%save(fullfile(dirDataBOLD, sprintf('Clustering_%s%sMovie123_new_masked_probability_critCorr1_eval.mat', cell2mat(setNameSubjNeural), 
%nameSubjBOLD)), ... %nameSubjNeural, nameSubjBOLD)),...
%        'eva_clusteringSU');
