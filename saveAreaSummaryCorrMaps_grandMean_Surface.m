% saveAreaSummaryCorrMaps_grandMean_Surface.m
%
% 2020/07/09 SHP
% modified from "saveAreaSummaryCorrMaps_Surface.m"
% Load a grand summary map of area summary maps from face patch neurons (corrMap_merged_FP) 
% ('/procdata/parksh/_macaque/CorrMap_SU_corticalFPMerged.mat')
% generated by "computeCorrMap_AllCells_FPAreaSummary.m" and save them onto surface
...
load('/procdata/parksh/_macaque/CorrMap_SU_AllCellsArt_corticalFPMerged.mat', 'info*', 'corrMap_merged_FP');
...
%     end
    ...
    for iMap = 1:3
        ...
        clear matR
        switch iMap
            case 1 % maximum of absolute correlation across neurons
                matR = corrMap_merged_FP.meanFractionAcrossArea; % corrMap_Area(iArea).matR_max;
                fileHead = 'MeanFractHighCorrCell';
            case 2 % average across neurons
                matR = corrMap_merged_FP.meanMaxAbs; %corrMap_Area(iArea).matR_avg;
                fileHead = 'MeanMaxAbs';
            case 3 % fraction of neurons with r>0.3 (or corrMap_Area(iArea).critCorr)
                matR = corrMap_merged_FP.grandMeanR; %corrMap_Area(iArea).fractionHighCorrCell;
                fileHead = 'MeanCorrMap';
        end
        ...
        for iMask = 1:2
            ...
            switch iMask
                case 1 % unmasked (everything within brain)
                    fileTail = '';
                    DSP.proc.fncvol_3d =  reshape(matR, [nx, ny, nz]); %mapR_Cluster(:,:,:,iK).*brainMask_BlockAna3D;
                case 2 % brainmasked (everything within brain)
                    fileTail = '_brainmask';
                    DSP.proc.fncvol_3d =  reshape(matR, [nx, ny, nz]).*brainMask_BlockAna3D; %mapR_Cluster(:,:,:,iK).*brainMask_BlockAna3D;
%                 case 3 % masked
%                     fileTail = '_moviemask';
%                     DSP.proc.fncvol_3d = reshape(matR, [nx, ny, nz]).*movieDrivenAmp.mask_amp1; %mapR_Cluster(:,:,:,iK).*movieDrivenAmp.mask_amp1; %reshape(mapR, [nx, ny, nz]).*movieDrivenAmp.mask_amp1;
            ...
            fname = sprintf('GrandFPAreaSummary_%s_AllCellsMovie123%s+orig.BRIK', fileHead, fileTail);%
