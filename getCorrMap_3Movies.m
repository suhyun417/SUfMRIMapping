function [S, dataBOLD] = getCorrMap_3Movies(saveFileName_matSDF, saveFileName) %(nameSubjNeural, nameSubjBOLD, saveFileName_matSDF, saveFileName)

% 3 Movies concatenated
...
fprintf(1, 'Selected Movie IDs: %s \n', num2str(setMovIDs))
fprintf(1, 'Chunked into sets of three concatenated movies \n')
numSet3Movies = length(setMovIDs)/3;
setMovIDs = reshape(setMovIDs, numSet3Movies, 3)'; % each row contains a set of 3 movies
...
for iCh=1:length(listSUchannelID) % go throucgh channel-by-channel     
     ...
     for iSM = 1:numSet3Movies
         % Get indices for common movies across cells
         indDataMov(iCh, :, iSM) = ismember(setMovIDs(iSM,:), str2num(char(tempListMov{iCh}))'); % data presence matrix (Channels x MovieIDs)
     end
end
...
%% Split fMRI responses into each set of movies (i.e. 3 movies concatenated)
...
for iSM = 1:numSet3Movies   
    ...
    [~, locMovBOLD] = intersect(listMovBOLD, setMovIDs(iSM, :));
    
    countTR = 0; tc=[];
    for iMov = 1:length(locMovBOLD) % concatenate across different movies
        fprintf(1, 'Movie ID: %d (%d/%d of the 3-movie set # %d/%d), Location in original BOLD: %d\n',...
            setMovIDs(iSM, iMov), iMov, length(locMovBOLD), iSM, numSet3Movies, locMovBOLD(iMov));
        ...
        countTR = size(tc, 4);
        %     dataBOLD.catmvoltc{iSM} = MCD.catmvoltc(:,:,:,lengthIndMovie_TR*(locMovBOLD(iMov)-1)+1:lengthIndMovie_TR*locMovBOLD(iMov)); % should be optimized for different set of movies
    end
    dataBOLD.catmvoltc{iSM} = tc;
    
    fprintf(1, 'Concatenated time course for movie ID [ %s ] has been saved in dataBOLD.catmvoltc{%d} \n',...
        num2str(setMovIDs(iSM,:)), iSM);
    fprintf(1, 'The number of TRs of dataBOLD.catmvoltc{%d} should be %d and is %d\n', ...
        iSM, lengthIndMovie_TR*3, size(dataBOLD.catmvoltc{iSM}, 4));
end
...
S = createCellRegressor_3Movies(dirDataNeural, setCellIDs, setMovIDs, indDataMov); %, flagConcat); 
% -- S(iChan, iSetMovie): for each cell (S.cellID) & each movie set (S.movID), 
...
% sp3=[];
for iC = 1:size(S,1) %length(S)
    fprintf(1, 'Channel %s (%d/%d) \n', S(iC, iSM).cellID, iC, size(S,1));
    
    for iSM = 1:size(S,2) % number of movie sets

        if isempty(S(iC, iSM).catmnsdf)
            continue;
        else
            fprintf(1, 'Channel %s (%d/%d), Movie [%s] \n', S(iC, iSM).cellID, iC, size(S,1), num2str(S(iC, iSM).movID));
            for iM = 1:length(S(iC, iSM).movID) %size(S,2) %length(setMovIDs)
                
%                 fprintf(1, 'Channel %s (%d/%d), Movie %d \n', S(iC).cellID, iC, size(S,1), setMovIDs(iM));
                %             S(iC, iM).rgrsMION_resample(1:onsetResp_numTR) = NaN;
                S(iC, iSM).rgrsMION_resample(lengthIndMovie_TR*(iM-1)+1:lengthIndMovie_TR*(iM-1)+onsetResp_numTR) = NaN;
...
[nx, ny, nz, nt] = size(dataBOLD.catmvoltc{1});
nVox = nx*ny*nz;

fprintf(1, 'Compute correlation using regressors:: \n')
for iC = 1:size(S,1) %length(S)
    for iSM = 1:size(S,2) %iM = 1:size(S,2)
        
        if ~isempty(S(iC, iSM).rgrsMION_resample)
            tic
            [Rvals, Pvals] = corr(reshape(dataBOLD.catmvoltc{iSM}, nVox, nt)', S(iC, iSM).rgrsMION_resample',...
                'rows','complete');
            
            S(iC, iSM).mapR = reshape(Rvals, [nx, ny, nz]).*(-1); % because of MION
            S(iC, iSM).mapP = reshape(Pvals, [nx, ny, nz]);
            fprintf(1, 'cell %s (%d/%d), movie set: %d [%s]\n', S(iC, iSM).cellID, iC, size(S,1), iSM, num2str(S(iC, iSM).movID)); %fprintf(1, '%d/%d: cell %s\n', iC, length(S), S(iC, iM).cellID)
            toc
        end
    end
end
...
tempS = S;
Names = fieldnames(S);
tempS = rmfield(tempS, Names([1:2, 4:end], 1)); %rmfield(tempS, 'matsdf'); %Names([1:2, 4:11], 1));
