% renameFiles_Dango.m
%
% Rename (and store) mat files containing Dango movie data collected by Kenji in January 2018, so that
% they can be matched to other files previously collected from other monkeys
% modified from renameFiles_Spice.m
%
% 2018/08/16 SHP
...
directory.source = fullfile(directory.dataNeural, '_orgData', 'Dango180123_26_movie');
directory.destination = fullfile(directory.dataNeural);

% read the excel file of cell list
filename_xls =  fullfile(directory.dataNeural, '_orgData/Dango_cell_list_180123_SHP.xls');
...
diary(fullfile(directory.destination, sprintf('renameFiles_Dango_%s', date)))
diary on
