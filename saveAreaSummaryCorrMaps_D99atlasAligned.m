% saveAreaSummaryCorrMaps_D99atlasAligned.m 
% modified from saveSUMaps_D99atlasAligned.m
%
% 2020/05/06 SHP
% Generate area summary maps in D99-atlas segmentation space 
% using afni function "3dresample" with -master option
%
% 1) Setup
%     1-1) Make a new directory under /procdata/parksh/_macaque/ to save the BRIK/HEAD files
%     1-2) Copy /procdata/parksh/_macaque/Art/Anatomy/_D99Atlas/e66_t1_avg_reg2_seg+orig to the new directory
%     1-3) Load the D99 segmentation of Artemis' brain (e66_t1_avg_reg2_seg+orig) and save the 3d volume
% 2) Copy the premade *_refit+orig in /procdata/parksh/_macaque/nameSubjBOLD/tempSURF to a new directory 
...
%         and saved in '/procdata/parksh/_macaque/nameSubjBOLD/tempSURF
...
dirSaveOutputFiles = fullfile(dirDataHome, sprintf('_MergedMaps%sD99Space', nameSubjBOLD));
...
if ~exist(fullfile(dirSaveOutputFiles, filename_segmat), 'file')    
    [err, voltc_seg, info_seg] = BrikLoad(fullfile(dirSaveOutputFiles, filename_seg));
    save(fullfile(dirSaveOutputFiles, sprintf('%sD99_ROIsegmentation.mat', nameSubjBOLD)), 'voltc_seg', 'info_seg')
else % if the file already exists
    load(fullfile(dirSaveOutputFiles, sprintf('%sD99_ROIsegmentation.mat', nameSubjBOLD)), 'info_seg')
end
...
d = dir(fullfile(dirOrgData, 'AreaSummary*Movie123_noFiltering_refit+orig.BRIK')); % only convert non-masked maps
...
infoFile_org = d;
 ...
 save(fullfile(dirDataHome, 'CorrMap_SU_AllCellsArt_corticalFPMerged_D99resample.mat'),...
     'catVoltc_new', 'catInfo_new', 'infoFile_org');
