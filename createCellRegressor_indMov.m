function S = createCellRegressor_indMov(dirDataNeural, setCellIDs, setMovIDs, sigma) %,indDataMov) %, flagConcat)

% 11/27/17 SHP added input "sigma" for computing SDF
...
d_n = d_n(cellfun(@length, {d_n(:).name})<50);
...
%   count = 0;
  for iMov = 1:length(setMovIDs) % should concatenate all those movies in order
      ...
      if ~indDataMov(strcmp(cur_cellID, listSUchannelID), iMov), 
          fprintf(1, 'cell: %s, movie: %d, data: %d \n', cur_cellID, cur_movID, indDataMov(strcmp(cur_cellID, listSUchannelID), iMov))
...
%       count = count+1;
      S(iChan, iMov).cellID = cur_cellID;
      S(iChan, iMov).movID = cur_movID;
      S(iChan, iMov).SUfilename = filename;
      [S(iChan, iMov).matsdf{1}, S(iChan, iMov).mnsdf] = computeMeanSDF(dirDataNeural, filename, sigma); % averaged SDF across trials
            
%       if ~flagConcat
          [S(iChan, iMov).rgrsMION, S(iChan, iMov).rgrsMION_resample] = computeMIONrgrs(S(iChan, iMov).mnsdf, timeResNeural_sec, fMRI_TR_sec);
%       end
...
%       %       S(iChan, iMov).catmnsdf = reshape(S(iChan, iMov).mnsdf,[1 prod(size(S(iChan, iMov).mnsdf))]); % concatenate across different movies
...
function [matSdf, mnsdf]= computeMeanSDF(dirDataNeural, filename, sigma)
...
for i=1:length(spikes)
  matSdf(:,i) = getSDF(spikes{i},win, sigma);
end
...
function sdf_out = getSDF(ts, win, sd)

% sd = 100; %1000; %2000; %1000; %2000;
