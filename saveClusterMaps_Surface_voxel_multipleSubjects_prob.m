...
load(fullfile(dirDataBOLD, sprintf('Clustering_%s%sMovie123_new_masked_probability_critCorr1.mat', cell2mat(setNameSubjNeural), nameSubjBOLD)))  % clustering based on the maps with movie-driven mask applied
% load(fullfile(dirDataBOLD, sprintf('Clustering_%s%sMovie123_new_masked_voxel_probability_critCorr2.mat', cell2mat(setNameSubjNeural), nameSubjBOLD)))  % clustering based on the maps with movie-driven mask applied
...
for iK = 1:length(paramClustering_global.setK)
    sortTargetK = paramClustering_global.setK(iK);
    ...
    [aa, locBest] = min(sum(Clustering_moviemask_valid.resultKMeans(sortTargetK-1).Vox_sumD)); % the best answer by K-means
    ...
    fname = sprintf('%s_VoxClustersCritCorr1_%d_Movie123_noFiltering+orig.BRIK', fileHead,sortTargetK);%
