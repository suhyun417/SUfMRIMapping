function blinks_data_positions = based_noise_blinks_detection_shp(pupil_data, sampling_rate_in_hz, blink_threshold_voltage) 
...
%     blinks      = vertcat(-1.*find(diff(pupil_data==0)==1), find(diff(pupil_data==0)==-1)+1);    
    blinks      = vertcat(-1.*find(diff(pupil_data < blink_threshold_voltage)==1), find(diff(pupil_data < blink_threshold_voltage)==-1)+1);    
    ...
    if(size(blinks, 1)>0 && blinks(1)>0) && pupil_data(1) < blink_threshold_voltage %pupil_data(1)==0 
        blinks = vertcat(0, blinks); %blinks = vertcat(blink_threshold_voltage-1, blinks); 
    end
    
    % Case 3: the data ends with a blink. In this case, blink offset will be defined as the last missing sample
    if(size(blinks, 1)>0 && blinks(end)<0) && pupil_data(end) < blink_threshold_voltage  %pupil_data(end)==0 
...
%     smooth_data(smooth_data==0) = nan;                      % replace zeros with NaN values
    smooth_data(smooth_data < blink_threshold_voltage) = nan;
