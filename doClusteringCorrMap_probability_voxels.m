...
for iK = 1:length(setK)
    ...
    Vox_indCluster_brainmask = NaN(sum(moviemask_vec), numRepeat);
...
%         SU_sumD_moviemask(:, iRep) = SUMD_SU;
        ...
        %     diary off
            ...
            Vox_indCluster_moviemask(:, iRep) = IDX_SU;
            Vox_sumD_moviemask(:, iRep) = SUMD_SU;
    ...
    Clustering_moviemask.resultKMeans(iK).Vox_sumD = SU_sumD_moviemask;
...
save(fullfile(dirDataNeural, sprintf('Clustering_%s%sMovie123_new_masked_probability_voxel_1.mat', nameSubjNeural, nameSubjBOLD)),...
