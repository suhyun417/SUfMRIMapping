...
nameSubjNeural = 'Spi'; %'Tor'; %'Rho'; % 'Sig'; %'Tor';
...
avgTS.meanTS = nanmean(matTS);
avgTS.medianTS = nanmedian(matTS);
avgTS.steTS = nanstd(matTS)./sqrt(size(matTS,1));
...
%
if flagSaveFile
    save(fullfile(dirDataNeural, sprintf('CorrMap_SUavg_%s%sMovie123_new.mat', nameSubjNeural, nameSubjBOLD)), 'avgTS', 'avgCorrMap', 'paramCorr');
    fprintf(1, '\n ...Results are saved in %s as %s \n', dirDataNeural, sprintf('CorrMap_SUavg_%s%sMovie123_new.mat', nameSubjNeural, nameSubjBOLD))
end
